...
@Service
public class MemphisReversalServiceImpl  implements MemphisReversalService {


    private static final Logger log = LoggerFactory.getLogger(MemphisReversalServiceImpl.class);
    ...
    public MemphisReversalServiceImpl(TblHistoricalRepository historicalRepository, MemphisHttpClient memphisHttpClient, TblConfigurationMemphisRepository tblConfigurationMemphisRepository, MemphisTransactionHistory txHistory) {
    ...
    @Override
    public AtenaResponse reversalById(AtenaTxByRequest atenaTxByRequest) {
        ...
        TblHistorical reversalTransaction = new TblHistorical();

        txHistory.saveInitializedFromAtenaRequest(reversalTransaction,originalTransaction, atenaTxByRequest);
        ...
        String memphisReversalResponseAsString=getMemphisTx(request,tokenResponse,timestamp,uriMemphis,commerceId,originalTransaction.getTicketNumber(),commerceName);


        txHistory.updateFromMemphisResponse(reversalTransaction, memphisReversalResponseAsString, "REVERSE");

        JsonObject jsonObjectCharge = JsonParser.parseString(memphisReversalResponseAsString).getAsJsonObject();
        ...
        MemphisApiTxResponse memphisApiTxResponse = new Gson().fromJson(memphisReversalResponseAsString, MemphisApiTxResponse.class);

        String authnum = "";

        Data data = new Data();
        AtenaResponse memphisReversalResponse = new AtenaResponse();

        if(memphisApiTxResponse.getResp_code().equals("00")){
            memphisReversalResponse.setSuccess(true);
            ...
            memphisReversalResponse.setSuccess(false);
            data.setResultCode(memphisApiTxResponse.getResp_code());
        }
        data.setApprovedCode(authnum);
        data.setDescription(memphisApiTxResponse.getDescription());

        memphisReversalResponse.setClaveOperacion(Long.valueOf(memphisApiTxResponse.getRequest_id()));
        memphisReversalResponse.setCode(memphisApiTxResponse.getResp_code());
        memphisReversalResponse.setDescription(memphisApiTxResponse.getDescription());
        memphisReversalResponse.setData(data);


        return memphisReversalResponse;
    }
    ...
    private String getMemphisTx(MemphisApiTxRequest refundRequest, String tokenResponse, String timestamp, String uriMemphis, String commerceId, String ticketNumber, String comerceName) {
        ...
        uriComplete += "reverse/" + ticketNumber;
